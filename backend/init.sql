...
-- -- Purana demo data saaf karo (Optional but recommended)
-- DELETE FROM employees;

-- -- 1. Commissioner (Admin) - Level 01
-- INSERT INTO employees (name, role, ward_id, phone_number, base_salary, integrity_score) 
-- VALUES ('Super Admin Commissioner', 'admin', 1, '1234567890', 250000.00, 100.00);

-- -- 2. Zonal Head - Level 02
-- INSERT INTO employees (name, role, ward_id, phone_number, base_salary, integrity_score) 
-- VALUES ('Zonal Head Officer', 'zonal', 2, '8000000001', 120000.00, 95.00);

-- -- 3. Supervisor - Level 03
-- INSERT INTO employees (name, role, ward_id, phone_number, base_salary, integrity_score) 
-- VALUES ('Amit Sharma (Supervisor)', 'supervisor', 1, '9988776655', 45000.00, 92.40);

-- -- 4. Field Worker - Level 04
-- INSERT INTO employees (name, role, ward_id, phone_number, base_salary, integrity_score) 
-- VALUES ('Rajesh Kumar (Worker)', 'worker', 3, '9000000001', 28500.00, 88.50);
...
-- 3. OTHER LOGS (Attendance, Ping, Salary, Audit)
CREATE TABLE attendance_logs (id SERIAL PRIMARY KEY, emp_id INT REFERENCES employees(id) ON DELETE CASCADE, check_in_time TIMESTAMPTZ DEFAULT NOW(), status VARCHAR(20));
CREATE TABLE ping_logs (id SERIAL PRIMARY KEY, emp_id INT REFERENCES employees(id) ON DELETE CASCADE, status VARCHAR(20) DEFAULT 'PENDING');
CREATE TABLE salary_ledger (id SERIAL PRIMARY KEY, emp_id INT REFERENCES employees(id) ON DELETE CASCADE, amount DECIMAL(10, 2), status VARCHAR(20) DEFAULT 'PENDING');
CREATE TABLE audit_logs (id SERIAL PRIMARY KEY, action_type VARCHAR(50), payload TEXT);

-- 🚀 SEED DATA (Strategic Mapping)
INSERT INTO wards (ward_name, lat, lng) VALUES ('Rohini', 28.7, 77.1), ('Lajpat Nagar', 28.5, 77.2), ('Civil Lines', 28.6, 77.2);

-- LEVEL 01 - Admin
INSERT INTO employees (name, role, ward_id, phone_number, base_salary) 
VALUES ('Super Admin Commissioner', 'admin', 1, '1234567890', 250000.00);

-- LEVEL 02 - Zonal Head
INSERT INTO employees (name, role, ward_id, phone_number, base_salary) 
VALUES ('Zonal Head Officer', 'zonal', 2, '8000000001', 120000.00);

-- LEVEL 03 - Supervisor
INSERT INTO employees (name, role, ward_id, phone_number, base_salary) 
VALUES ('Amit Sharma (Supervisor)', 'supervisor', 1, '9988776655', 45000.00);

-- LEVEL 04 - Worker
INSERT INTO employees (name, role, ward_id, phone_number, base_salary, last_transfer_date) 
VALUES ('Rajesh Kumar (Worker)', 'worker', 3, '9000000001', 28500.00, '2022-01-01');

-- 🔄 OTP & Simulation Logic (Using phone_number instead of ID for safety)
UPDATE employees SET current_otp = '123456' WHERE phone_number = '9000000001';